...
public class MunkalapKezelo extends Kezelo<Munkalap> {
    ...
    public void hozzaad() {
        try {
            ...
            for(int i = 0; i < SzervizMain.getSzerelok().size(); i++) {
                System.out.println( (i+1) + ": " + SzervizMain.getSzerelok().get(i) );
            }
            System.out.println("Szerelo sorszama: ");
            munkalap.setSzerelo( SzervizMain.getSzerelok().get( SzervizMain.bekerSzam() - 1 ) );
